...
def search_endpoint(q: str | None = None, county: str | None = None, limit: int = 20,
    ...
    try:
        with get_db_session() as session:
            ...
            where = []
            params: Dict[str, Any] = {}
            # Do not require geometry for basic list search; map will skip items without lat/lon
            if q:
                like_parts = []
                ...
                # Allow county and id matches too
                if 'county' in colset:
                    like_parts.append("wr.county ILIKE :q")
                like_parts.append("wr.id ILIKE :q")
                if like_parts:
